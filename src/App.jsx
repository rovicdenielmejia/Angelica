    ...
    <div className="invite-floral-canvas min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-md mx-auto w-full px-4 space-y-8 pb-8">
        ...
        <section className="space-y-4">
          <div className="relative rounded-2xl p-4 shadow-soft" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
                  ...
                  <div className="max-h-[calc(50vh-48px)] overflow-y-auto">
                    {searchResults.map((guest, index) => (
                      <button
                        ...
                        className="w-full px-4 py-3 text-left hover:bg-white/40 transition-colors flex items-center justify-between gap-3"
                        style={{
                          borderTop: index > 0 ? '1px solid var(--color-border-light)' : 'none',
                          background: index === selectedIndex ? 'var(--color-accent-muted)' : 'transparent',
                        }}
                      >
                        ...
                        <svg className="w-4 h-4 shrink-0 text-secondary opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                        </svg>
                      </button>
                    ))}
                  </div>
                  <div className="px-4 py-2 border-t" style={{ background: 'var(--color-surface-subtle)', borderColor: 'var(--color-border-light)' }}>
          ...
          {searchQuery.trim() && !showDropdown && (
            <section className="space-y-4 animate-fade-in-up">
              {searchResults.length > 0 ? (
                ...
                <div className="rounded-2xl p-6 text-center border shadow-soft" style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}>
